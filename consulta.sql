...
-- fazer o backup da tabela--

create table backup_funcionarios select * from funcionarios;
select * from backup_funcionarios;

-- filtrar dados --

select nome, salario from funcionarios where salario > 3000;

select nome, data_admissao from funcionarios where year(data_admissao) < 2020;

-- filtrar por ordenação --

select nome, cargo, salario from funcionarios order by salario desc , nome;
...
WHERE id_produtos > 20;
    ...
    create table backup_produtos select * from produtos;
   desc backup_produtos;
   truncate backup_produtos;
	insert into backup_produtos select * from produtos;
...
SELECT * FROM produtos WHERE categoria = 'prato principal' LIMIT 2 OFFSET 4;
    
    
    -- Selecione os 5 produtos mais caros do restaurante --
    
    select id_produtos, nome, preco from produtos order by preco desc limit 5;
    ...
    select nome, descricao from produtos order by nome, descricao;
    
    -- Selecione o id do produto e os ingredientes de informações de produto para os ingredientes que contenham a palavra “carne” --
    
select id_produtos,nome from produtos where nome like '%carne%';
    
    -- Selecione o nome e a descricao dos produtos que tem o preço superior a 30 --
    
    select nome, descricao from produtos where preco > 30;
